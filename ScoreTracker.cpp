...
ScoreTracker::ScoreTracker(SC2Data *sc2, QMainWindow *window): Observer(sc2){ 
	mw = window;
...
void ScoreTracker::attach(ScoreWriter* sw) {
	watchers.push_back(sw);
}
...
void ScoreTracker::addConfirmMessage(player* playerA, player* playerB) {
	...
	if(cfg->popupsEnabled) {
		...
		sprintf_s(playerAButton, "%s: %s (%s)", playerA->race, playerA->name, playerA->result);
		...
		sprintf_s(playerBButton, "%s: %s (%s)", playerB->race, playerB->name, playerB->result);
